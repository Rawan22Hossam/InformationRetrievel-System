...
# read files name
files_name = os.listdir('D:\ProjectIR\doc')
...
), index=get_term_freq(document_of_terms[0]).keys())  # type: ignore

for i in range(1, len(document_of_terms)):
    term_freq[i] = get_term_freq(document_of_terms[i]).values()

term_freq.columns = ['doc'+str(i) for i in range(1, 11)]  # type: ignore
print('TF')
print(term_freq)
...
def insert_query(q):
    ...
    query['tf'] = [1 if x in preprocessing(
        q) else 0 for x in list(normalized_term_freq_idf.index)]
